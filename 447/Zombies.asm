.data
x: .byte 'x' #For building the mze LEDs are lit where 'x' is present
start: .byte 0 #Broken tried to use this to start program with 'b' button
Ppos: .byte 0,0 #player postion
oldtime: .word 64 #Time of prior move
dir: .byte 0,1,2,3 #Player direction
currtime: .word 64 #Current time
Zpos: .byte 0,6,61,2,6,62,3,63,62 #starting positions of each Zombie
Zdir: .byte 0,0,0,0 #Zombie Direction
...
#main polling loop of the program, loops checking for key presses and checks time to move zombies	
poll:	
	move $a0,$zero
	move $a1,$zero
	#Gets system time				
...
#handles zombie 1 movement left
movezl:
la $t1,Zdir
lb $t2,0($t1)
beq $t2,3,makemovez
...
#handles zombie 1 rightward movement
movezr:
la $t1,Zdir
lb $t2,0($t1)
beq $t2,1,makemovez
...
#handles zombie 1 upward movement
movezu:
la $t1,Zdir
lb $t2,0($t1)
beq $t2,4,makemovez
...
#handles zombie 1 downward movement
movezd:
la $t1,Zdir
lb $t2,0($t1)
beq $t2,2,makemovez
...
#handles zombie 2 left movement
movezl2:
la $t1,Zdir
lb $t2,1($t1)
beq $t2,3,makemovez2
...
jal makemovez3
#handles zombie 2 right movement
movezr2:
la $t1,Zdir
lb $t2,1($t1)
beq $t2,1,makemovez2
...
#handles zombie 4 left movement
movezl4:
la $t1,Zdir
lb $t2,3($t1)
beq $t2,3,makemovez4
...
jal poll
#handles Zombie 4 right movement
movezr4:
la $t1,Zdir
lb $t2,3($t1)
beq $t2,1,makemovez3
...
sb $t1,3($t2)
jal poll
#handles zombie 4 upward movement
movezu4:
la $t1,Zdir
lb $t2,3($t1)
beq $t2,4,makemovez4
...
#handles zombie 4 downward movement
movezd4:
la $t1,Zdir
lb $t2,3($t1)
beq $t2,2,makemovez4
